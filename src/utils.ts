...
function formatOutput(output: string): Promise<string> {
	return new Promise((resolve) => {
		try {
			JSON.parse(output);
			exec(`echo '${output.replace(/'/g, "'\\''")}' | jq .`, (error, stdout, stderr) => {
				if (error) {
			...
			exec(`echo '${output.replace(/'/g, "'\\''")}' | prettier --parser html`, (error, stdout, stderr) => {
				if (error) {
...
export async function executeHurl(options: HurlExecutionOptions): Promise<string> {
	// Set status bar message
	const statusBarMessage = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 10000);
	...
	const output = await new Promise<string>((resolve, reject) => {
		exec(command, (error, stdout, stderr) => {
			...
			if (stderr) {
				...
				reject(new Error("Oops! Something went wrong while running Hurl", {
